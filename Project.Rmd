...
pacman::p_load(tidyverse, ggplot2, dplyr, cluster, arules, arulesViz, lubridate, factoextra)
```
...
# Prepare transactions: Replace StockCode with Description
transactions <- retail_clean %>%
  select(InvoiceNo, Description) %>%
  distinct() %>%  # Ensure unique InvoiceNo-Description pairs
  group_by(InvoiceNo) %>%
...
# Customize Visualization: Smaller and Attractive Labels
plot(
  top_rules,
  method = "graph",
  measure = "support",
  shading = "lift",
  control = list(
    type = "items",              # Use item-based visualization
    nodeSize = 10,               # Adjust the size of nodes
    edgeCol = "grey",            # Edge color
    labelCol = "black",          # Label color
    labelSize = 0.7,             # Label size (smaller text)
    main = "Top Association Rules"
  )
)
...
# Improve labels: Shorten or wrap text
labels <- labels(top_rules)
short_labels <- str_wrap(labels, width = 20)  # Wrap labels to fit within 20 characters

# Visualize the graph with improved layout and labels
plot(
  top_rules,
  method = "graph",
  measure = "support",
  ...
  control = list(
    type = "items",              # Use item-based visualization
    nodeSize = 10,               # Adjust node size
    edgeCol = "darkgrey",        # Make edges more visible
    labelSize = 
```
